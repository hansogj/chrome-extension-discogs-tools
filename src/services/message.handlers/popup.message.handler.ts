...
const sendMessage = (id = 0, body: ActionTypes) =>
  Promise.race([
    asPromised((resolver) => chrome.tabs.sendMessage(id, body, resolver)).catch(() =>
      chrome.runtime.sendMessage(body),
    ),

    asPromised((resolver) => chrome.runtime.sendMessage(body, resolver)).catch(() =>
      chrome.tabs.sendMessage(id, body),
    ),
  ]);
