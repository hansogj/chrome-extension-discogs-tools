...
function addWantlistBtnToModalBox(boxHeader) {
    if (boxHeader && !document.getElementById('wantlistbtn')) {
        boxHeader.innerHTML =
            '<button id="wantlistbtn" class="button button_small button_blue"  onclick="javascript:window.discogs.addWantList(this, \'PUT\')">Add all to wantlist</button> <br />' +
...
find(document, 'strong.pagination_total').forEach(function(paginationTotal) {
    document.title = document
        .querySelector('strong.pagination_total')
        .textContent.match(/of\s([\d|\,]*)/)[1]
})
...
setTimeout(function() {
    setInterval(function() {
        find(document, '.react-modal-header').forEach(addWantlistBtnToModalBox)
    }, 500)
}, 2000)
