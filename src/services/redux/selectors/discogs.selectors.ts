...
export const getReleasePageId = createSelector(
  getReleasePageItem,
  (releasePageItem) =>
...
export const getReleasePageMaster = createSelector(
  getReleasePageItem,
  (releasePageItem) =>
    maybe(releasePageItem).mapTo('master').valueOr(undefined) as MasterRelease,
)

export const fromReleasePageMaster = (prop: keyof MasterRelease) => {
  return createSelector(getReleasePageMaster, (master) => {
    console.log(master, prop)
    return maybe(master).mapTo(prop).valueOr(undefined)
  })
}
