...
module.exports = function () {
  (function (w) {
    ...
    function getReleaseLinks(release) {
      return find(release, ".release_title_link a")
        .filter(function (a) {return a.href.match(releasePattern).length;})
    }
    ...
    function getArtist(release) {
      ...
      find(release, ".release_title.set_height a")
        ...
        .map(function (a) {return a.innerText;}).pop();
    }

    function buildModel(all, release) {
      return getReleaseLinks(release).map(function (link) {
        return {
    ...
    function uniqueRelease (release, index, list) {
      ...
      function comparator (_release, _index) {return release.title.toLowerCase() === _release.title.toLowerCase() && index < _index};
    ...
    function addCss() {
      ...
      var css = 'tr {display:table-row; transition: all .2s ease-out; }' +
            '.hidden {display:none;}',
    ...
    var hidden = colums.map(buildModel.bind(this, colums))
    ...
    console.log(hidden.map(function (rel) {return rel.artist + ": " + rel.title;}));
