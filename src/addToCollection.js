...
function addNotes(notes) {
    find(document, ".notes_field[data-field='3'] .notes_show").forEach(
        clickElem
    )
...
function addToFolder(release_id) {
    if (release_id !== undefined) {
        return xhrJson('POST', 'https://www.discogs.com/_rest/collection', {
...
function listCollection(filter) {
    return find(document.querySelector('.release_list_table'), 'tr').filter(
        filter
    )
}
...
function listFormatCollection(format) {
    return listCollection(function(row) {
        return find(row, '[data-header=Format]').filter(function(format_show) {
            return format_show.innerText.match(format)
        }).length
    })
}
...
function listNotedCollection(notes) {
    return listCollection(function(row) {
        return find(row, '.notes_show').filter(function(notes_show) {
            return notes_show.innerText.match(notes)
        }).length
    })
}

function check(row) {
    return find(row, 'input[type=checkbox]').map(function(input) {
